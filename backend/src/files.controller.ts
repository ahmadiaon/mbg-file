...
export class FilesController {
  ...
  @Get() async list(@Req() req: any, @Query('q') q?: string, @Query('folderId') folderId?: string) {
    requireUser(req);
    const files = await this.prisma.file.findMany({ where: { folderId: folderId ? Number(folderId) : null, ...(q ? { originalFilename: { contains: q } } : {}) }, include: { folder: true }, orderBy: { createdAt: 'desc' } });
